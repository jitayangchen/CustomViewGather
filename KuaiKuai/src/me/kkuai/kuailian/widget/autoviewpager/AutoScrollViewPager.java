package me.kkuai.kuailian.widget.autoviewpager;
...
public class AutoScrollViewPager extends ViewPager {
    ...
    private boolean isActionDownEvent = true;
...
//        heightMeasureSpec = MeasureSpec.makeMeasureSpec(height, MeasureSpec.EXACTLY);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			if (Math.abs(startX - ev.getX()) > 10) {
				isActionDownEvent = false;
			}
			break;
		case MotionEvent.ACTION_UP:
			if (isActionDownEvent) {
				if (null != itemClickListener) {
					itemClickListener.onItemClick(getCurrentItem());
				}
			} else {
				isActionDownEvent = true;
			}
			break;
